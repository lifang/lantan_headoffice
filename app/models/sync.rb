...
class Sync < ActiveRecord::Base
  ...
  def self.out_data(time)
    ...
    dirs.each_with_index {|dir,index| Dir.mkdir path+dirs[0..index].join   unless File.directory? path+dirs[0..index].join }
    models=['product.rb', 'image_url.rb', 'sale.rb', 'capital.rb', 'car_brand.rb', 'car_model.rb', 'material_order.rb', 'customer.rb', 'sv_card.rb', 'c_svc_relation.rb', 'svcard_prod_relation.rb', 'as_car_num.rb', 'as_customer_num_relation.rb', 'as_reservation.rb', 'as_res_prod_relation.rb']

...
class SvCardsController < ApplicationController   #优惠卡控制器
  ...
  def index #优惠卡主页
    @sv_cards = SvCard.where("status = #{SvCard::STATUS[:NORMAL]}").order("created_at desc").paginate(:page => params[:page] ||= 1,:per_page => 10)
  end
  ...
  def create          #创建优惠卡
    ...
    if SvCard.where(["types= ? and name = ? and status = ?", card_type, card_name, SvCard::STATUS[:NORMAL]]).blank?
      if card_type == 1                                       #如果是储值卡
        started_money = params[:started_money].to_f
        ended_money = params[:ended_money].to_f
        sv_card.update_attributes(:name => card_name, :types => card_type, :price => started_money,
                                  :description => card_description, :status => SvCard::STATUS[:NORMAL])
        if sv_card.save
        ...
        sv_card.update_attributes(:name => card_name, :types => card_type,:discount => discount, :price => price, 
                                  :description => card_description, :status => SvCard::STATUS[:NORMAL])
        sv_card.save
      end
      begin
  ...
  def update    #更新优惠卡
    ...
    if SvCard.where(["id != ? and types = ? and name = ? and status = ?", sc.id, type, name, SvCard::STATUS[:NORMAL]]).blank?
      if type == 1
  ...
  def destroy   #删除优惠卡
    ...
    if sv_card.update_attribute("status", SvCard::STATUS[:DELETED])
      flash[:notice] = "删除成功!"
    else
      flash[:notice] = "删除失败!"
    end

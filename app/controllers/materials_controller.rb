...
class MaterialsController < ApplicationController   #库存控制器
  ...
  def index
    ...
    @mat_orders = MaterialOrder.is_headoffice_not_canceled.where(status).where(started_time).where(ended_time).paginate(:page => params[:page] ||= 1, :per_page => 2) if @tab.nil? || @tab.eql?("mat_orders_tab")
